...
function TestLibraryScreen() {
  ...
  return (
    <div
      className={`bg-white min-h-screen pb-10 transition-all duration-1000 ${
        isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
      }`}
    >
      {/* Header Section */}
      <div className="mx-4 md:mx-8 lg:mx-16 mb-8">
        <h2 className="text-3xl font-bold pb-4 pt-8 text-center text-[#469B74]">
          Test Library
        </h2>
      </div>
      ...
      <div className="mx-4 md:mx-8 lg:mx-16 mb-8">
        <Row gutter={[24, 24]}>
          {testData.map((test) => (
            <Col key={test.id} xs={24} sm={12} md={6}>
              <TestCard test={test} />
            </Col>
          ))}
        </Row>
