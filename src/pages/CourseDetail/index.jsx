...
import { CheckCircle, Clock, BookOpen, GraduationCap, ChevronDown, ChevronUp, Lock } from "lucide-react"
                ...
                {expandedSections[section.title] && (
                  <ul className="section-lessons">
                    {section.subSections?.map((subSection, subSectionIndex) => {
                      const isAccessible = isSubSectionAccessible(sectionIndex, subSectionIndex)
                      return (
                        <li
                          key={subSectionIndex}
          ...
          <div className="main-content">
            ...
            {selectedLesson && (
              <div className="current-lesson-header mb-4">
                <h2 className="font-shopee current-lesson-title text-2xl">Bài học hiện tại: {selectedLesson.title}</h2>
                <span className="text-sm text-gray-500">Phần: {selectedLesson.sectionTitle}</span>
              </div>
            )}
            ...
            <div className="font-shopee content-section bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-yellow-800 font-semibold">Xem trước miễn phí</h3>
              </div>
              <p className="text-yellow-700">
                Bạn đang xem bài học đầu tiên của khóa học. Để truy cập toàn bộ {totalLessons} bài học và tài liệu, vui
                lòng mua khóa học.
              </p>
            </div>
                ...
                <div key={index} className="mb-4">
                  ...
                  <ul className="ml-4">
                    {section.subSections?.map((subSection, subIndex) => (
                      <li key={subIndex} className="mb-1 flex items-center gap-2">
                        <span>• {subSection.title}</span>
                        {!isSubSectionAccessible(index, subIndex) && <Lock className="w-3 h-3 text-gray-400" />}
                      </li>
                    ))}
                  </ul>
                </div>
        ...
        <div className="purchase-modal-content">
          ...
          <div className="bg-gradient-to-r from-[#469B74] to-[#5BAE88] text-white p-6 -m-6 mb-6 rounded-t-lg">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                ...
                <p className="text-white/90 text-sm">Bạn sắp sở hữu khóa học tuyệt vời này!</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-6 mb-6">
            ...
            <div className="flex-1">
              <h3 className="text-xl font-semibold text-gray-800 mb-2 line-clamp-2">{courseData?.name}</h3>
              <p className="text-gray-600 text-sm mb-3 line-clamp-2">{courseData?.description}</p>
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="bg-red-100 text-red-600 text-xs px-2 py-1 rounded-full font-medium">Giá ưu đãi</span>
                </div>
              </div>
              <div className="text-right">
                <p className="text-gray-500 text-sm line-through">$299</p>
                <p className="text-green-600 text-sm font-medium">Tiết kiệm 40%</p>
              </div>
            </div>
          ...
          <div className="flex items-center gap-3 mb-6 p-4 bg-gray-50 rounded-lg">
            ...
            />
            <div>
              <p className="font-medium text-gray-800">{instructorData?.fullName || "Giảng viên"}</p>
              <p className="text-sm text-gray-600">Chuyên gia hàng đầu</p>
            </div>
            <div className="ml-auto text-right">
              <div className="flex items-center gap-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  <CheckCircle key={star} className="w-4 h-4 text-yellow-400 fill-current" />
                ))}
              </div>
          ...
          <div className="flex gap-3">
            ...
            >
              {buyMutation.isPending ? "Đang xử lý..." : "Mua ngay - Bắt đầu học"}
            </Button>
          </div>

          {/* Security Notice */}
          <div className="text-center mt-4">
            <p className="text-xs text-gray-500">🔒 Thanh toán an toàn và bảo mật • Hỗ trợ 24/7 • Hoàn tiền dễ dàng</p>
          </div>
        </div>
      </Modal>
    </CourseDetailStyle>
