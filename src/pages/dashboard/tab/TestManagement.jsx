...
export default function TestManagement() {
  ...
  const fetchTests = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `http://https://safeeduapi-dev.sit/api/tests?${queryParams.toString()}`
      );
  ...
  const handleDelete = async (id) => {
    try {
      ...
      await fetch(
        `http://https://safeeduapi-dev.sit/api/tests/${id}?typeId=${filterType || ""}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleModalOk = async () => {
    try {
      ...
      if (editingId) {
        ...
        await fetch(
          `http://https://safeeduapi-dev.sit/api/tests/${editingId}?typeId=${
            filterType || ""
          }`,
          {
            method: "PUT",
        ...
        // Add new test
        const response = await fetch(
          `http://https://safeeduapi-dev.sit/api/tests?typeId=${filterType || ""}`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(testData),
